(() => {
  ...
  const build = () => {
    const host = document.querySelector('#wgBrowse'); if (!host) return;
    const rows = matches();
    host.querySelector('.wg-browse-count').textContent = `${rows.length} ${city()} ${rows.length===1?'place':'places'}`;
    host.querySelector('.wg-browse-grid').innerHTML = rows.length ? rows.map(card).join('') : '<div class="wg-browse-empty"><strong>No places match those filters.</strong><p>Clear one or more filters and try again.</p></div>';
    host.querySelectorAll('.wg-browse-card').forEach(c => {
      const p = rows.find(x => String(x.id ?? x.name) === String(c.dataset.browseId));
      c.addEventListener('click', e => { if (e.target.closest('button')) return; open(p); });
      c.querySelector('.wg-browse-heart')?.addEventListener('click', e => { e.stopPropagation(); const a=saved(); const i=a.indexOf(p.name); if(i>=0){a.splice(i,1);e.currentTarget.textContent='♡';toast('Removed from saved places')}else{a.push(p.name);e.currentTarget.textContent='♥';toast('Saved locally')} localStorage.setItem(saveKey,JSON.stringify([...new Set(a)].slice(0,100))); });
      c.querySelector('.wg-browse-add')?.addEventListener('click', e => { e.stopPropagation(); if(window.WORTHGO_DAY?.add){window.WORTHGO_DAY.add(p);toast('Added to My Day')}else toast('My Day is unavailable'); });
    });
  };
  const mount = () => {
    if (document.querySelector('#wgBrowse')) return;
    const finder = document.querySelector('.finder'); if (!finder) return;
    const section = document.createElement('section'); section.id='wgBrowse'; section.className='wg-browse shell';
    section.innerHTML = `<div class="wg-browse-head"><div><span class="kicker">EXPLORE THE CITY</span><h2>All places in ${city()}</h2><p>Browse everything first. Then narrow it down to what fits you.</p></div><span class="wg-browse-count"></span></div><div class="wg-searchbar"><span>⌕</span><input id="wgBrowseSearch" type="search" placeholder="Search restaurants, food, areas or things to do…" autocomplete="off"><button type="button" id="wgBrowseClear">Clear</button></div><div class="wg-chip-row" id="wgCategoryChips"></div><div class="wg-filterbar"><button type="button" class="wg-filter-toggle" id="wgFilterToggle">☷ Filters</button><select id="wgSort">${options(['worth','rating','price','name'],'Sort: WorthGo picks')}</select><span class="wg-active-count" id="wgActiveCount">No filters</span></div><div class="wg-filter-panel" id="wgFilterPanel"><div><label>Venue<select id="wgVenue">${options([], 'Any venue')}</select></label><label>Food<select id="wgFood">${options([], 'Any food')}</select></label><label>When<select id="wgMeal">${options([], 'Any time')}</select></label><label>Experience<select id="wgExperience">${options([], 'Any experience')}</select></label><label>Budget<select id="wgBudget">${options([], 'Any budget')}</select></label><label>Area<select id="wgArea">${options(cfg().areas.slice(1), 'Any area')}</select></label></div><button type="button" class="wg-clear-filters" id="wgClearFilters">Clear all filters</button></div><div class="wg-browse-grid"></div>`;
    finder.after(section);
    wire(section); build();
  };
  const wire = host => {
    const rows=all();
