(() => {
  ...
  const removeDuplicateTimeQuickAction=()=>{document.querySelectorAll('.quick-row [data-quick="two-hours"]').forEach(el=>el.remove());};
  ...
  const init=()=>{
    ...
    protectFilterScroll();
    let queued=false;
    const observer=new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;syncSaveButtons();fixBuildDay();installFilterGoUX();updateFilterHint();removeDuplicateTimeQuickAction()})});
    observer.observe(document.body,{childList:true,subtree:true});
    syncSaveButtons();
    fixBuildDay();
    installFilterGoUX();
    updateFilterHint();
    removeDuplicateTimeQuickAction();
